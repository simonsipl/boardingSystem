import sorting from './ByRowStrategy';
import paxes from '../../paxes';

test('is sorting arrays properly', () => {
    const RowStrategy = sorting;
    expect(RowStrategy.sortSeats(paxes)).toEqual(
...
test('throws error if not array', () => {
    const RowStrategy = sorting;
    expect(() => RowStrategy.sortSeats('test')).toThrow('Seat list is not an array');
